...
export const AIAgentStatus: React.FC<AIAgentStatusProps> = ({
  ...
  const { connectionStatus, connectWebSocket, disconnectWebSocket } = useAppContext();
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics(prev => ({
        ...
        memory_usage: Math.max(40, Math.min(90, prev.memory_usage + (Math.random() - 0.5) * 5)),
        active_workflows: Math.max(0, Math.min(10, prev.active_workflows + Math.floor((Math.random() - 0.5) * 3))),
        queue_size: Math.max(0, Math.min(20, prev.queue_size + Math.floor((Math.random() - 0.5) * 4))),
        last_heartbeat: new Date().toISOString()
      }));
    }, 5000);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={cn(
                ...
                connectionStatus === 'connected' ? 'border-green-500 bg-green-50' : 'border-gray-300 bg-gray-50'
              )}>
                <Bot className={cn(
                  "w-5 h-5",
            ...
            <div className="text-right">
              <p className="text-sm font-medium">{metrics.active_workflows} active</p>
              <p className="text-xs text-gray-500">{metrics.queue_size} queued</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className={cn(
              ...
              healthStatus === 'warning' ? 'border-yellow-500 bg-yellow-50' :
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-600">{metrics.active_workflows} workflows</p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-600">{metrics.queue_size} pending</p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-600">{(metrics.success_rate * 100).toFixed(0)}%</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm text-gray-600">{metrics.cpu_usage.toFixed(0)}%</span>
            </div>
            <Progress 
              value={metrics.cpu_usage} 
              className={cn(
                "h-2",
                metrics.cpu_usage > 80 && "bg-red-100"
              )}
            />
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm text-gray-600">{metrics.memory_usage.toFixed(0)}%</span>
            </div>
            <Progress 
              value={metrics.memory_usage} 
              className={cn(
                "h-2",
                metrics.memory_usage > 85 && "bg-red-100"
              )}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-4 border-t">
          <div>
            ...
            <p className="text-lg font-bold text-blue-600">{metrics.avg_processing_time}s</p>
          </div>
          <div>
        ...
        <div className="text-xs text-gray-500 pt-2 border-t">
          Last heartbeat: {formatDistanceToNow(new Date(metrics.last_heartbeat), { addSuffix: true })}
        </div>
      </CardContent>
    </Card>
  );
